  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border bg-background/90 backdrop-blur-sm">
      <div className="mx-auto max-w-screen-xl px-6 h-14 flex items-center gap-4">
        ...
        {/* Search — docs only, hidden on mobile */}
          {inDocs && (
          <button
            type="button"
            onClick={openSearch}
        ...
        <div className="flex items-center gap-1 ml-auto sm:hidden">
          {inDocs && (
            <button
              ...
              onClick={openSearch}
              aria-label="Search docs"
              className="flex items-center justify-center w-8 h-8 rounded-[3px] text-fg-secondary hover:text-foreground hover:bg-surface transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-1"
            >
      ...
      {mobileOpen && (
        <nav
          ...
          className="sm:hidden border-t border-border bg-background/95 backdrop-blur-sm px-4 py-3 flex flex-col gap-0.5"
        >
          {navLinks.map(({ label, href, match }) => {
            const active = match(pathname);
            return (
