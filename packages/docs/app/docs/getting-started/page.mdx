...
```bash
pnpm add @dave/react @dave/tokens @dave/charts
```
...
const nextConfig = {
  transpilePackages: ['@dave/react', '@dave/charts', 'recharts'],
};
...
module.exports = {
  content: [
    ...
    './node_modules/@dave/charts/dist/**/*.js',
  ],
  theme: {
    extend: {
      fontFamily: {
...
```tsx
import { Button, Card, CardHeader, CardTitle, CardContent } from '@dave/react';
...
In your consumer app, add a `pnpm.overrides` entry so `@dave/react` resolves `@dave/tokens` to the local tarball rather than trying the registry:

```json
{
  "pnpm": {
    "overrides": {
      "@dave/tokens": "file:/tmp/dave-tarballs/dave-tokens-0.1.0.tgz"
    }
  }
}
```
